...
if strcmp(player,'Coal Company') == 1
    ...
    MW_ADD = 50; % this can be customized later
    [mc] = coalsteam(N,MW_ADD);% this is the marginal cost of the player's generation plan 
    cs = mc;
    bidprice = 50; % this can be customized later
else
    % Annual Costs for Gas Turbine Plants
    MW_ADD = 51; % this can be customized later
    [mc] = gasturbine(N,MW_ADD);% this is the marginal cost of the player's generation plan
    bidprice = 65; 
    if N <= 5
     bidprice = 65; 
    else
...
[baseMVA, bus, gen, branch, success] = runuopf('wscc9bus',options);
branch = success;
%<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
%                                   Forecast Price
%--------------------------------------------------------------------------
currentprice = historicalprice(1,2);
...
fprintf('\n');
fprintf('\n   Player:  %s',player);
fprintf('\n                            Total Profits: %s',int2str(profits));
